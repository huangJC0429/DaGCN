...
parser.add_argument('--dataset', default='CS', help='Dataset string.')
...
if args.dataset in ["Computers", "Photo", "CS", "Physics"]:
    ...
    edge_index, features, idx_train, idx_val, idx_test, labels = data.edge_index, data.x, data.train_mask, data.val_mask, data.test_mask, data.y
...
aug_features = np.hstack((features, X_aug)) # np.multiply(features, X_aug)

knn = KNN_Rewiring(edge_index, aug_features, 8, device, idx_train, args)
