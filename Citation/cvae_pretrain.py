...
# def feature_tensor_normalize2(feature):
#     norm =  1e-6 + torch.sum(feature, dim=0)
#     # print(norm.shape)
...
def generated_generator(args, device, adj, features, labels, features_normalized, adj_normalized, idx_train,edge_index=None, fmodel='gcn'):
    F_MASK = np.load("train_friend_mask.npy").astype(int)
    x_list, c_list = [], [] 
    ...
    for _ in range(int(epochs / 2)):
        ...
        if fmodel == 'gcn':
            ...
            output = model(features_normalized, edge_index)
        ...
        for _, (x, c) in enumerate(tqdm(cvae_dataset_dataloader)):
            ...
            for i in range(args.num_models):
                if fmodel == 'gcn':
                    ...
                    logits = model(augmented_features, edge_index)
                ...
                if t > args.warmup:
                    ...
                    for i in range(args.update_epochs):
                        ...
                        if fmodel == 'gcn':
                            ...
                            output = model(best_augmented_features, edge_index)
                        output = torch.log_softmax(output, dim=1)
                        loss_train = F.nll_loss(output[idx_train], labels[idx_train])
                        loss_train.backward()
                        model_optimizer.step()
